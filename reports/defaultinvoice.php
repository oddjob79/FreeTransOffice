...
// invoice title
		$section->addText('Invoice', $largeBoldBlackFont, $titleStyle);
...
// closing text
		...
		$dirname = $this->getFileLocation();
		$invoicenumber = $invheader['invoice_number'];
		$filename = 'inv_'.$invoicenumber;
		$reportformat = $invheader['report_format_id'];
		
		// file created in word format and downloaded
		if ($reportformat == 2) {   // Word
			$objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'Word2007');
			$filedoc = $dirname.'/'.$filename.'.docx';
		...
		} else { // Default to PDF if Word not chosen

			// Saving the document as HTML file...
			$objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'HTML');
			$filehtml = $dirname.'/'.$filename.'.html';
			$objWriter->save($filehtml);

			$htmlfilepath = file_get_contents($filehtml);
			...
			$dompdf->render();
			$file_to_save = $dirname.'/'.$filename.'.pdf';
//			$file_to_save = $filepdf;
